...
export class TokenService {
  ...
  async sign(payload): Promise<Auth.AccessRefreshTokens> {
    ...
    // User tokens are removed before new tokens are created because we want to ensure that only one token is valid at a time
    await this.tokenRepository.removeUserTokens(userId);
  ...
  async getAccessTokenFromWhitelist(
    ...
    await this.tokenRepository.removeExpiredTokens();
    // check if token is in the whitelist and not expired yet
  ...
  async refreshTokens(
    ...
    await this.tokenRepository.removeUserTokens(_payload.id); // User tokens are removed before new tokens are created because we want to ensure that only one token is valid at a time
