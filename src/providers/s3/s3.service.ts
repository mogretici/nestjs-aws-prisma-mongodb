...
export class S3Service {
  ...
  async uploadFile(
    ...
    if (file.mimetype.startsWith('image')) {
      ...
      const pdfConvert = new PdfConvert(file.buffer);
      const firstPage = await pdfConvert.convertPageToImage(1, {
        resolution: 70,
      });
      if (firstPage) {
        await this.s3Client.send(
      ...
      console.log(
        'S3 Service: File is not an image or PDF, no thumbnail will be generated.',
      );
    }
